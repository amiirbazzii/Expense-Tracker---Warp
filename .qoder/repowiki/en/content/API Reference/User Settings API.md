...
- TypeScript interfaces for Currency and Calendar types
...
CreateNew --> SetDefaults["currency=USD, calendar=gregorian"]
...
**Integration Flow**
1. SettingsProvider uses `useQuery` to fetch current settings
2. Settings are made available to all components via React Context
3. `useMutation` enables settings updates from any component
4. UI automatically re-renders when settings change
