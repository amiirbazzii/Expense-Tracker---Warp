...
export default function DashboardPage() {
  ...
  const { categories: categoriesAll, forValues: forValuesAll } =
    useOfflineFirstData();
  const expenseCategoryNames: string[] = useMemo(
    () => (categoriesAll || []).map((c: any) => c.name).filter(Boolean),
    [categoriesAll],
  );
  const forValueNames: string[] = useMemo(
    () => (forValuesAll || []).map((f: any) => f.value).filter(Boolean),
    [forValuesAll],
  );
  ...
  const { categoryTotalsForMode, dailyTotalsForMode, totalForMode } =
    useMemo(() => {
      if (mode === "income") {
        let list = (effIncome || []).filter(
          (item: any) => item && item.category !== "Card Transfer",
        );
        // category filter applies to income as well
        if (filters.categories.length > 0) {
          list = list.filter((it: any) =>
            filters.categories.includes(
              Array.isArray(it.category) ? it.category[0] : it.category,
            ),
          );
        }
      ...
      let list = (effExpenses || []).filter((expense: any) => {
        ...
        const catOk =
          filters.categories.length > 0
            ? categories.some((c: string) => filters.categories.includes(c))
            : true;
