...
const POPOVER_CLS = "bg-white rounded-2xl shadow-xl border border-gray-200/60 overflow-hidden";
...
export function GregorianDatePicker({ value, onChange, label }: GregorianDatePickerProps) {
  ...
  const [currentDate, setCurrentDate] = useState<DateObject>(() => {
    if (value) return new DateObject({ date: value, calendar: gregorian });
    return new DateObject({ calendar: gregorian });
  });
  ...
  const open = useCallback(() => {
    if (value) {
      ...
      setTempDate(d);
      setCurrentDate(new DateObject({ date: value, calendar: gregorian }));
    } else {
      setTempDate(null);
    }
    setIsOpen(true);
  ...
  const goBack = useCallback(() => {
    setCurrentDate((prev) => {
      const next = new DateObject(prev);
  ...
  const handleMonthYearChange = useCallback((d: DateObject) => {
    setCurrentDate(new DateObject(d));
    setTempDate((prev) => {
      if (!prev) return prev;
      const next = new DateObject(prev);
        ...
        <div ref={popoverRef} style={popoverStyle} className={POPOVER_CLS}>
          ...
          <div className={`p-3 border-t border-gray-100 ${FONT}`}>
            <button
              ...
              className="w-full py-2.5 bg-black text-white text-sm font-medium rounded-xl hover:bg-gray-800 active:scale-[0.98] transition-all duration-150"
            >
              Confirm
            </button>
          </div>
