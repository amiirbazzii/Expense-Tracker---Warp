...
jest.mock("localforage", () => ({
  config: jest.fn((...args: any[]) => mockLocalforage.config(...args)),
  getItem: jest.fn((...args: any[]) => mockLocalforage.getItem(...args)),
  setItem: jest.fn((...args: any[]) => mockLocalforage.setItem(...args)),
  removeItem: jest.fn((...args: any[]) => mockLocalforage.removeItem(...args)),
  clear: jest.fn((...args: any[]) => mockLocalforage.clear(...args)),
  keys: jest.fn((...args: any[]) => mockLocalforage.keys(...args)),
  createInstance: jest.fn(() => mockLocalforage),
}));
...
describe("ConflictDetector", () => {
  ...
  test("should detect corrupted local data", async () => {
    ...
    const cloudData: CloudDataMapping = {
      expenses: [{ _id: "exp1", amount: 25.5, title: "Coffee" }],
      income: [],
      categories: [],
      cards: [],
      forValues: [],
  ...
  test("should validate data hash consistency", async () => {
    ...
    const isValid = await conflictDetector.validateDataHash(
      localHash,
      cloudHash,
    );
    expect(isValid).toBe(true);

    const isInvalid = await conflictDetector.validateDataHash(
      localHash,
      "xyz789",
    );
  ...
  test("should generate consistent hash", () => {
    const data1 = { amount: 25.5, title: "Coffee" };
    const data2 = { amount: 25.5, title: "Coffee" };
    const data3 = { amount: 30.0, title: "Coffee" };
...
describe("CloudSyncManager", () => {
  ...
  test("should retry failed operations with exponential backoff", async () => {
    ...
    const result = await (cloudSyncManager as any).retryWithBackoff(
      mockOperation,
    );
...
describe("LocalFirstConvexClient", () => {
  ...
  test("should initialize with user ID", async () => {
    ...
    expect(LocalStorageManager.prototype.initialize).toHaveBeenCalledWith(
      mockUserId,
    );
  });
  ...
  test("should create expense locally first", async () => {
    ...
    jest
      .spyOn(LocalStorageManager.prototype, "saveExpense")
      .mockResolvedValue(mockLocalExpense);
    jest
      .spyOn(LocalStorageManager.prototype, "addPendingOperation")
      .mockResolvedValue();
    ...
    expect(LocalStorageManager.prototype.saveExpense).toHaveBeenCalledWith(
      expenseData,
    );
    expect(
      LocalStorageManager.prototype.addPendingOperation,
    ).toHaveBeenCalled();
  });
  ...
  test("should get expenses from local storage", async () => {
    ...
    jest
      .spyOn(LocalStorageManager.prototype, "getExpenses")
...
describe("Integration Tests", () => {
  test("should complete full offline-to-online sync cycle", async () => {
    ...
    const localFirstClient = new LocalFirstConvexClient(
    ...
    jest.spyOn(LocalStorageManager.prototype, "saveExpense").mockResolvedValue({
      ...expenseData,
      id: "exp1",
      localId: "local1",
      syncStatus: "pending",
    ...
    jest.spyOn(CloudSyncManager.prototype, "syncToCloud").mockResolvedValue({
      success: true,
      conflicts: [],
      errors: [],
      syncedCount: 1,
  ...
  test("should handle performance under load", async () => {
    ...
    for (let i = 0; i < 100; i++) {
      promises.push(
        localFirstClient.createExpense({
          amount: 10 + i,
          title: `Expense ${i}`,
          category: ["Test"],
          for: ["Test"],
