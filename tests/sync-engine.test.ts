...
function buildConvexArgs(storeName: string, action: string, payload: any): Record<string, any> {
  const base = { ...payload };
  delete base.id;
  delete base.localId;
...
describe('Sync Engine — Queue Processing Logic', () => {
  ...
  describe('buildConvexArgs', () => {
    ...
    it('should handle income CREATE/UPDATE/DELETE', () => {
      const createPayload = { id: 'inc_1', amount: 200, source: 'Salary', category: 'Work', date: 1000, cardId: 'card_1' };
      ...
      const updateArgs = buildConvexArgs('income', 'UPDATE', { id: 'inc_1', amount: 250 });
    ...
    it('should handle categories and forValues', () => {
      const catArgs = buildConvexArgs('categories', 'CREATE', { id: 'cat_1', name: 'Food' });
      expect(catArgs.name).toBe('Food');
      expect(catArgs.id).toBeUndefined();

      const fvArgs = buildConvexArgs('forValues', 'CREATE', { id: 'fv_1', value: 'Friend' });
      expect(fvArgs.value).toBe('Friend');
    });

    it('should handle cards CREATE/DELETE', () => {
      const createArgs = buildConvexArgs('cards', 'CREATE', { id: 'card_1', name: 'Visa' });
  ...
  describe('markEntityAsSynced', () => {
    it('should update syncStatus to synced and set lastSyncedAt', async () => {
      const expense = await manager.saveExpense({ amount: 50, title: 'Test', category: ['Food'], for: ['Self'], date: Date.now() });
    ...
    it('should not fail if entity does not exist', async () => {
      await expect(manager.markEntityAsSynced('expenses', 'nonexistent')).resolves.not.toThrow();
    });
  });
  ...
  describe('FIFO Queue Processing Simulation', () => {
    ...
    it('should leave failed mutations in queue and skip past non-retryable ones', async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 5));
      await manager.enqueue('CREATE', 'expenses', { amount: 50, title: 'Good' });
